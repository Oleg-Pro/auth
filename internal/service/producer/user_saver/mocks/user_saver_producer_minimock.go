...
//go:generate minimock -i github.com/Oleg-Pro/auth/internal/service/producer/user_saver.UserSaverProducer -o user_saver_producer_minimock.go -n UserSaverProducerMock -p mocks
...
// UserSaverProducerMock implements mm_user_saver.UserSaverProducer
type UserSaverProducerMock struct {
	t          minimock.Tester
	finishOnce sync.Once

	funcSend          func(ctx context.Context, info *model.UserInfo) (err error)
	funcSendOrigin    string
	inspectFuncSend   func(ctx context.Context, info *model.UserInfo)
	afterSendCounter  uint64
	beforeSendCounter uint64
	SendMock          mUserSaverProducerMockSend
}

// NewUserSaverProducerMock returns a mock for mm_user_saver.UserSaverProducer
...
func (mmSend *mUserSaverProducerMockSend) Expect(ctx context.Context, info *model.UserInfo) *mUserSaverProducerMockSend {
	...
	mmSend.defaultExpectation.params = &UserSaverProducerMockSendParams{ctx, info}
...
// ExpectInfoParam2 sets up expected param info for UserSaverProducer.Send
func (mmSend *mUserSaverProducerMockSend) ExpectInfoParam2(info *model.UserInfo) *mUserSaverProducerMockSend {
	if mmSend.mock.funcSend != nil {
		mmSend.mock.t.Fatalf("UserSaverProducerMock.Send mock is already set by Set")
	}
	...
	mmSend.defaultExpectation.paramPtrs.info = &info
	mmSend.defaultExpectation.expectationOrigins.originInfo = minimock.CallerInfo(1)

	return mmSend
}

// Inspect accepts an inspector function that has same arguments as the UserSaverProducer.Send
func (mmSend *mUserSaverProducerMockSend) Inspect(f func(ctx context.Context, info *model.UserInfo)) *mUserSaverProducerMockSend {
...
func (mmSend *mUserSaverProducerMockSend) Set(f func(ctx context.Context, info *model.UserInfo) (err error)) *UserSaverProducerMock {
	if mmSend.defaultExpectation != nil {
...
// Then helper
func (mmSend *mUserSaverProducerMockSend) When(ctx context.Context, info *model.UserInfo) *UserSaverProducerMockSendExpectation {
	if mmSend.mock.funcSend != nil {
		mmSend.mock.t.Fatalf("UserSaverProducerMock.Send mock is already set by Set")
	}
	...
	expectation := &UserSaverProducerMockSendExpectation{
		mock:               mmSend.mock,
		params:             &UserSaverProducerMockSendParams{ctx, info},
...
// Send implements mm_user_saver.UserSaverProducer
func (mmSend *UserSaverProducerMock) Send(ctx context.Context, info *model.UserInfo) (err error) {
	...
	mm_params := UserSaverProducerMockSendParams{ctx, info}
	...
	if mmSend.SendMock.defaultExpectation != nil {
		...
		mm_got := UserSaverProducerMockSendParams{ctx, info}
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.info != nil && !minimock.Equal(*mm_want_ptrs.info, mm_got.info) {
				mmSend.t.Errorf("UserSaverProducerMock.Send got unexpected parameter info, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
					mmSend.SendMock.defaultExpectation.expectationOrigins.originInfo, *mm_want_ptrs.info, mm_got.info, minimock.Diff(*mm_want_ptrs.info, mm_got.info))
			}
	...
	mmSend.t.Fatalf("Unexpected call to UserSaverProducerMock.Send. %v %v", ctx, info)
	return
}
